...
namespace RazorPagesApp.Pages
{
    public class kitchenModel : PageModel
    {
        ...
        public async Task OnGet() // тестить на IIS: public void OnGet()
        {
            ...
            ChartData.Columns.Add("Температура", typeof(System.String));
            ChartData.Columns.Add("t", typeof(System.Single));
            // Add rows to data table
            for (int i = 0; i < SensorData_05.Count; i++)
            {
                ChartData.Rows.Add($"{SensorData_05[i].date.ToShortTimeString()}, {(SensorData_05[i].date.ToShortDateString()).Substring(0, 5)}", (SensorData_05[i].temp)); ChartData.Rows.Add(SensorData_05[i].date.ToShortTimeString(), (SensorData_05[i].temp));
            }
            ...
            // Set Chart Title
            column.Caption.Text = "Температура";
            // Set chart sub title
            //column.SubCaption.Text = "2017-2018";
            // hide chart Legend
            column.Legend.Show = false;
            // set XAxis Text
            column.XAxis.Text = "Время";
            // Set YAxis title
            column.YAxis.Text = "Температура (t)";
            // set chart theme
